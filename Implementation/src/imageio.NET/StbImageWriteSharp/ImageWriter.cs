        ...
        internal static partial class StbImageWriteSharp
        {
            ...
            public unsafe class ImageWriter
            {
                ...
                private void CheckParams(byte[] data, int width, int height, ColorComponents components)
                {
                    ...
                    {
                        throw new ArgumentException(string.Format("Not enough data. 'data' variable should contain at least {0} bytes.", requiredDataSize));
                    }
                }
                ...
                public void WritePng(byte[] data, int width, int height, ColorComponents components, Stream dest)
                {
                    ...
                    fixed(byte* b = &data[0])
                    {
                        WritePng((void*)b, width, height, components, dest);
                    }
                }
