...
namespace System
{
    ...
    namespace AI
    {
        ...
        internal static partial class MKL
        {
            ...
            public static void Frac(Half[] x, Half[] y)
            {
                ...
                Parallel.For(0, Threads, (int i) =>
                {
                    ...
                    {
                        y[j] = x[j] - (Half)Math.Truncate(x[j]);
                    }
                });
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public static void Frac(float[] x, float[] y)
            {
                var Threads = Environment.ProcessorCount;
                var TaskPart = x.Length / Threads;
                var TaskRemainder = x.Length % Threads;
                Parallel.For(0, Threads, (int i) =>
                {
                    ...
                    for(int j = start; j < end; j++)
                    {
                        y[j] = x[j] - (float)Math.Truncate(x[j]);
                    }
                });
                var start_ = TaskPart * Threads;
                var end_ = start_ + TaskRemainder;
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public static void Frac(double[] x, double[] y)
            {
                var Threads = Environment.ProcessorCount;
                var TaskPart = x.Length / Threads;
                var TaskRemainder = x.Length % Threads;
