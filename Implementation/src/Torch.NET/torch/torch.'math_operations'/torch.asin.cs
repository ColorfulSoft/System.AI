...
public static partial class torch
{
    ...
    public static Tensor asin(Tensor x)
    {
        ...
        switch(Result.dtype)
        {
            ...
            {
                throw new torch.TorchException("TorchException: torch.asin is not implemented for bool and integer tensors.");
            }
        }
        if(x.requires_grad)
        {
                    ...
                    case torch.dtype.half:
                    {
                        ...
                        Parallel.For(0, Threads, (int tp) =>
                        {
                            for(int i = tp * TaskPart; i < (tp + 1) * TaskPart; i++)
                            {
                                var vDW = destDW[i];
                                var vW = srcW[i];
                                srcDW[i] += (Half)(1f / (Math.Sqrt(1f - vW * vW)) * vDW);
                            }
                        });
                    ...
                    case torch.dtype.@float:
                    {
                        ...
                        Parallel.For(0, Threads, (int tp) =>
                        {
                            for(int i = tp * TaskPart; i < (tp + 1) * TaskPart; i++)
                            {
                                var vDW = destDW[i];
                                var vW = srcW[i];
                                srcDW[i] += (float)(1f / (Math.Sqrt(1f - vW * vW)) * vDW);
                            }
                        });
                        for(int i = Threads * TaskPart; i < srcDW.Length; i++)
                        {
                    ...
                    case torch.dtype.@double:
                    {
                        ...
                        Parallel.For(0, Threads, (int tp) =>
                        {
                            for(int i = tp * TaskPart; i < (tp + 1) * TaskPart; i++)
                            {
                                var vDW = destDW[i];
                                var vW = srcW[i];
                                srcDW[i] += (1.0 / (Math.Sqrt(1.0 - vW * vW)) * vDW);
                            }
                        });
                        for(int i = Threads * TaskPart; i < srcDW.Length; i++)
                        {
