...
public static partial class torch
{
    ...
    public static Tensor tanh(Tensor x)
    {
        ...
        switch(Result.dtype)
        {
            ...
            {
                throw new torch.TorchException("TorchException: torch.tanh is not implemented for int8 tensors.");
            }
            case torch.dtype.uint8:
            {
                throw new torch.TorchException("TorchException: torch.tanh is not implemented for uint8 tensors.");
            }
            case torch.dtype.int16:
            {
                throw new torch.TorchException("TorchException: torch.tanh is not implemented for short(int16) tensors.");
            }
            case torch.dtype.int32:
            {
                throw new torch.TorchException("TorchException: torch.tanh is not implemented for int(int32) tensors.");
            }
            case torch.dtype.int64:
            {
                throw new torch.TorchException("TorchException: torch.tanh is not implemented for long(int64) tensors.");
            }
            case torch.dtype.@bool:
            {
                throw new torch.TorchException("TorchException: torch.tanh is not implemented for bool tensors.");
            }
        }
        if(x.requires_grad)
        {
            Result.__backward_fn = () =>
            {
                switch(x.dtype)
                {
                    case torch.dtype.float16:
                    {
                        ...
                        var destDW = Result.__grad_float16;
                        var Threads = Environment.ProcessorCount;
                    ...
                    case torch.dtype.float64:
                    {
                        ...
                        var destDW = Result.__grad_float64;
                        var Threads = Environment.ProcessorCount;
                        var TaskPart = destW.Length / Threads;
                        Parallel.For(0, Threads, (int tp) =>
                        {
