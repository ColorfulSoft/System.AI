...
public static partial class torch
{
    ...
    public static Tensor cosh(Tensor x)
    {
        ...
        switch(Result.dtype)
        {
            ...
            {
                throw new torch.TorchException("TorchException: torch.cosh is not implemented for bool and integer tensors.");
            }
        }
        if(x.requires_grad)
        {
                    ...
                    case torch.dtype.@float:
                    {
                        ...
                        Parallel.For(0, Threads, (int tp) =>
                        {
                            for(int i = tp * TaskPart; i < (tp + 1) * TaskPart; i++)
                            {
                                var vDW = destDW[i];
                                var vW = srcW[i];
                                srcDW[i] += (float)(Math.Sinh(vW) * vDW);
                            }
                        });
                        for(int i = Threads * TaskPart; i < srcDW.Length; i++)
                        {
