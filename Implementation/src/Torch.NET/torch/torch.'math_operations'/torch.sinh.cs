...
public static partial class torch
{
    ...
    public static Tensor sinh(Tensor x)
    {
        ...
        switch(Result.dtype)
        {
            ...
            {
                throw new torch.TorchException("TorchException: torch.sinh is not implemented for bool and integer tensors.");
            }
        }
        if(x.requires_grad)
        {
                    ...
                    case torch.dtype.float32:
                    {
                        ...
                        Parallel.For(0, Threads, (int tp) =>
                        {
                            for(int i = tp * TaskPart; i < (tp + 1) * TaskPart; i++)
                            {
                                var vDW = destDW[i];
                                var vW = srcW[i];
                                srcDW[i] += (float)(Math.Cosh(vW) * vDW);
                            }
                        });
                        for(int i = Threads * TaskPart; i < srcDW.Length; i++)
                        {
