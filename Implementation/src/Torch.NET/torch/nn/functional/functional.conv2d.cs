        ...
        public static partial class functional
        {
            ...
            public static Tensor conv2d(Tensor input, Tensor weight, Tensor bias = null, int[] stride = null, int[] padding = null, int[] dilation = null, int groups = 1)
            {
                ...
                switch(Result.dtype)
                {
                            ...
                            Parallel.For(0, outD, (int depth) =>
                            {
                                for(int height = 0; height < outH; height++)
                                {
                                    for(int width = 0; width < outW; width++)
                                    {
                                        for(int batch = 0; batch < outB; batch++)
                                        {
                                            Result.__data_float16[(((height * outW) + width) * outD + depth) * outB + batch] += bias.__data_float16[depth];
                                        }
                                    }
                                }
                            });
                            ...
                            Parallel.For(0, outD, (int depth) =>
                            {
                                for(int height = 0; height < outH; height++)
                                {
                                    for(int width = 0; width < outW; width++)
                                    {
                                        for(int batch = 0; batch < outB; batch++)
                                        {
                                            Result.__data_float64[(((height * outW) + width) * outD + depth) * outB + batch] += bias.__data_float64[depth];
                                        }
                                    }
                                }
                            });
                            ...
                            Parallel.For(0, outD, (int depth) =>
                            {
                                for(int height = 0; height < outH; height++)
                                {
                                    for(int width = 0; width < outW; width++)
                                    {
                                        for(int batch = 0; batch < outB; batch++)
                                        {
                                            Result.__data_int8[(((height * outW) + width) * outD + depth) * outB + batch] += bias.__data_int8[depth];
                                        }
                                    }
                                }
                            });
                            ...
                            Parallel.For(0, outD, (int depth) =>
                            {
                                for(int height = 0; height < outH; height++)
                                {
                                    for(int width = 0; width < outW; width++)
                                    {
                                        for(int batch = 0; batch < outB; batch++)
                                        {
                                            Result.__data_int16[(((height * outW) + width) * outD + depth) * outB + batch] += bias.__data_int16[depth];
                                        }
                                    }
                                }
                            });
                            ...
                            Parallel.For(0, outD, (int depth) =>
                            {
                                for(int height = 0; height < outH; height++)
                                {
                                    for(int width = 0; width < outW; width++)
                                    {
                                        for(int batch = 0; batch < outB; batch++)
                                        {
                                            Result.__data_int32[(((height * outW) + width) * outD + depth) * outB + batch] += bias.__data_int32[depth];
                                        }
                                    }
                                }
                            });
                    ...
                    {
                        throw new torch.TorchException("TorchException: nn.functional.conv2d is not implemented for bool tensors.");
                    }
                }
                    ...
                    Result.__backward_fn = () =>
                    {
                        switch(input.dtype)
                        {
                            ...
                            case torch.dtype.half:
                            {
                                ...
                                var inputData = input.__data_float16;
                                var resultGrad = Result.__grad_float16;
                                                    ...
                                                    for(int f_height = 0; f_height < kernelH; f_height++)
                                                    {
                                                        ...
                                                        for(int f_width = 0; f_width < kernelW; f_width++)
                                                        {
                                                            ...
                                                            for(int f_depth = 0; f_depth < kernelD; f_depth++)
                                                            {
                                                                for(int depth = kernelD * group; depth < kernelD * (group + 1); depth++)
                                                                {
                                                                    var i1 = (((iy * inW) + ix) * inD + depth) * inB + batch;
                                                                    var i2 = (((f_height * kernelW) + f_width) * kernelD + f_depth) * kernelN + o_depth;
                                                                    inputGrad[i1] += kernelData[i2] * chain_grad;
                                                                    kernelGrad[i2] += inputData[i1] * chain_grad;
                                                                }
                                                            }
                                                        }
                                                    }
                                ...
                                var biasGrad = bias.__grad_float16;
                                Parallel.For(0, outB, (int batch) =>
                                {
                                    for(int height = 0; height < outH; height++)
                                    {
                            ...
                            case torch.dtype.@double:
                            {
                                ...
                                var inputData = input.__data_float64;
                                var resultGrad = Result.__grad_float64;
                                Parallel.For(0, outB, (int batch) =>
                                {
                                    for(int height = 0; height < outH; height++)
                                    {
                                ...
                                var biasGrad = bias.__grad_float64;
                                Parallel.For(0, outB, (int batch) =>
                                {
                                    for(int height = 0; height < outH; height++)
                                    {
